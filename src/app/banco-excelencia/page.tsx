  ...
  return (
    <div className="space-y-12">
      ...
      <section className="bg-gradient-to-r from-[#001141] to-[#0A1A4A] border border-white/5 p-8 rounded-2xl relative overflow-hidden">
        ...
        {principios.length === 0 ? (
          <div className="bg-[#001141]/80 backdrop-blur-sm p-6 rounded-xl border border-[#0F62FE]/30 text-center relative z-10">
            <h3 className="text-lg font-bold text-white mb-2">Nenhum princípio cadastrado</h3>
            <p className="text-sm text-gray-400">
               Os princípios pedagógicos inegociáveis devem ser cadastrados no banco de dados (tabela <code>principios_pedagogicos</code>) para que a IA os utilize como base estrutural.
            </p>
          </div>
        ) : (
      ...
      <section className="space-y-6">
        ...
        {casosSucesso.length === 0 ? (
          <div className="bg-[#0A1A4A] border border-white/5 p-8 rounded-xl text-center">
             <div className="mx-auto w-12 h-12 bg-[#198038]/20 rounded-full flex items-center justify-center mb-4">
                <Target className="w-6 h-6 text-[#198038]" />
             </div>
             <h3 className="text-xl font-bold text-white mb-2">Sem exemplos reais injetados ainda</h3>
             <p className="text-gray-400 max-w-lg mx-auto">
               Os cases de sucesso dos professores precisam ser inputados na ferramenta e salvos na tabela <code>banco_excelencia</code> para aparecerem nesta seção. <br/><br/>
               <span className="text-[#0F62FE] font-medium">Como funciona:</span> Quando os professores aplicam planos bem sucedidos, esses dados alimentam a IA da Tom, garantindo que os futuros planos gerados repliquem as estratégias que deram certo nas nossas escolas.
             </p>
          </div>
        ) : (
